...
from views import insert_dock, list_docks, retrieve_dock, delete_dock, update_dock
from views import insert_hauler, list_haulers, retrieve_hauler, delete_hauler, update_hauler
from views import list_ships, retrieve_ship, delete_ship, update_ship, insert_ship
    ...
    def do_POST(self):
        ...
        if url["requested_resource"] == "ships":
            if pk != 0:
                ...
                successfully_updated = insert_ship(request_body)
                if successfully_updated:
                    return self.response(
                        "", status.HTTP_204_SUCCESS_NO_RESPONSE_BODY.value
                    )
            ...
            if pk != 0:
                ...
                successfully_updated = insert_hauler(request_body)
                if successfully_updated:
                    return self.response(
                        "", status.HTTP_204_SUCCESS_NO_RESPONSE_BODY.value
                    )
        if url["requested_resource"] == "docks":
            if pk != 0:
                ...
                successfully_updated = insert_dock(request_body)
                if successfully_updated:
                    return self.response(
                        "", status.HTTP_204_SUCCESS_NO_RESPONSE_BODY.value
                    )
